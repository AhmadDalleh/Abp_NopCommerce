...
* Check the `ConnectionStrings` in `appsettings.json` files under the `NopCommerceV1.Web` and `NopCommerceV1.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 2682e5fa-703a-4668-9a84-a5e519a553ff
```

> `2682e5fa-703a-4668-9a84-a5e519a553ff` is the password of the certificate, you can change it to any password you want.
...
Run `NopCommerceV1.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `NopCommerceV1.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `NopCommerceV1.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
